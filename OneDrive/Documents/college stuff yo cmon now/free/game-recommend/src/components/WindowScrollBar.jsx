...
export function WindowScrollBar() {
  ...
  const syncMetrics = useEffectEvent(() => {
    ...
    const thumbSize = maxScroll
      ? Math.max(MIN_THUMB_SIZE, Math.min(trackHeight, (viewportHeight / root.scrollHeight) * trackHeight))
      : trackHeight;
    const maxThumbOffset = Math.max(trackHeight - thumbSize, 0);
    const thumbOffset = maxScroll ? (window.scrollY / maxScroll) * maxThumbOffset : 0;
  ...
  return (
    <div className="window-scrollbar-shell" aria-hidden="true">
      <div className="retro-scrollbar window-retro-scrollbar">
        <button
          type="button"
          className="retro-scroll-button retro-scroll-button-up"
          onClick={() => scrollByAmount(-DEFAULT_SCROLL_STEP)}
        ...
        <button
          type="button"
          className="retro-scroll-button retro-scroll-button-down"
          onClick={() => scrollByAmount(DEFAULT_SCROLL_STEP)}
          disabled={!metrics.canScroll}
          tabIndex={-1}
        />
